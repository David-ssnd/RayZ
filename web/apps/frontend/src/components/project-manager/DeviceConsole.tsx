...
export function DeviceConsole() {
  ...
  return (
    <div className="flex flex-col h-[400px] border rounded-md bg-zinc-950 text-zinc-50 font-mono text-sm">
      <div className="flex items-center justify-between px-4 py-2 border-b bg-zinc-900 rounded-t-md">
        <div className="flex items-center gap-2">
          <Terminal className="w-4 h-4" />
          <span className="font-semibold">Device Console</span>
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-6 w-6 p-0 hover:bg-zinc-800 hover:text-red-400"
          onClick={clearLog}
          title="Clear log"
        >
      ...
      <ScrollArea className="flex-1 p-4" ref={scrollRef}>
        <div className="space-y-1.5">
          {messageLog.length === 0 ? (
            ...
            messageLog.map((log) => (
              <div key={log.id} className="flex gap-2 items-start group hover:bg-zinc-900/50 p-0.5 rounded">
                ...
                <div className="flex-1 min-w-0 break-all">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className={`text-xs font-bold ${log.direction === 'in' ? 'text-cyan-400' : 'text-green-400'}`}>
                  ...
                  <div className="text-zinc-300 opacity-90">
